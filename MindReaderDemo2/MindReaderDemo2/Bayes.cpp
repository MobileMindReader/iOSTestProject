...
//    Mat w = cv::Mat(numBasisFuncs+1, 1, CV_32F, some);
...
//    Mat x = Mat(1, numSamples, CV_32F, trainX);
...
//    Mat targetNoise = Mat(1, numSamples, CV_32F, targetNoises);
    
    
//    Mat(1, numSamples, CV_32F, Scalar(0.0));
...
//    Mat y = Mat(x.rows, x.cols, CV_32F);
    Mat y = w.data.fl[0] * Mat::ones(x.rows, x.cols, CV_32F);
